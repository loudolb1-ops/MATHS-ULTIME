...
// ── Témoignages Maths Ultime V1 (dans /public/temoignages-v1/) ─────────────
const V1_IMAGES = [
  'temoignage-v1-01.jpg',
  'temoignage-v1-02.jpg',
  'temoignage-v1-03.jpg',
  'temoignage-v1-04.jpg',
  'temoignage-v1-05.jpg',
  'temoignage-v1-06.jpg',
  'temoignage-v1-07.png',
  'temoignage-v1-08.png',
  'temoignage-v1-09.png',
  'temoignage-v1-10.png',
  'temoignage-v1-11.png',
];
...
const BLUE_STONE_THEME: CarouselTheme = {
  cardGradient: 'linear-gradient(160deg, #ede5da 0%, #d8ccbc 40%, #e8ddd0 70%, #cfc3b4 100%)',
  border: '#8a7968',
  shadow: '#5a4e3e',
  ornament: '#d4a017',
  frieze: '#8a7968',
...
const ORANGE_STONE_THEME: CarouselTheme = {
  cardGradient: 'linear-gradient(160deg, #f3ded0 0%, #e0bda0 40%, #f0d4bd 70%, #d9ac88 100%)',
  border: '#a3603a',
  shadow: '#6b3218',
  ornament: '#e0611a',
  frieze: '#a3603a',
...
function CommentCarousel({
  ...
  useEffect(() => {
    ...
    if (trackRef.current) {
      posRef.current = reverse ? trackRef.current.scrollWidth / 2 : 0;
    }
    ...
    function tick(now: number) {
      ...
      if (!paused && trackRef.current) {
        ...
        const deltaPx = (SPEED_PX_PER_SEC * delta) / 1000;
        if (reverse) {
          ...
          >
            ...
            <div
              ...
              style={{
                ...
                boxShadow: `4px 4px 0 ${theme.shadow}, inset 0 1px 0 rgba(255,255,255,0.45)`,
                cursor: 'default',
                position: 'relative',
              }}
            >
              {/* Ornures dans les coins — style grec */}
              {/* Coin haut-gauche */}
              <svg style={{ position: 'absolute', top: 6, left: 6, width: 18, height: 18, opacity: 0.75 }} viewBox="0 0 18 18" fill="none">
                <path d="M2 16 L2 2 L16 2" stroke={theme.border} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M2 10 L5 10 L5 5 L10 5" stroke={theme.ornament} strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" opacity="0.8"/>
              </svg>
              {/* Coin haut-droit */}
              <svg style={{ position: 'absolute', top: 6, right: 6, width: 18, height: 18, opacity: 0.75 }} viewBox="0 0 18 18" fill="none">
                <path d="M16 16 L16 2 L2 2" stroke={theme.border} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M16 10 L13 10 L13 5 L8 5" stroke={theme.ornament} strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" opacity="0.8"/>
              </svg>
              {/* Coin bas-gauche */}
              <svg style={{ position: 'absolute', bottom: 6, left: 6, width: 18, height: 18, opacity: 0.75 }} viewBox="0 0 18 18" fill="none">
                <path d="M2 2 L2 16 L16 16" stroke={theme.border} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M2 8 L5 8 L5 13 L10 13" stroke={theme.ornament} strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" opacity="0.8"/>
              </svg>
              {/* Coin bas-droit */}
              <svg style={{ position: 'absolute', bottom: 6, right: 6, width: 18, height: 18, opacity: 0.75 }} viewBox="0 0 18 18" fill="none">
                <path d="M16 2 L16 16 L2 16" stroke={theme.border} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M16 8 L13 8 L13 13 L8 13" stroke={theme.ornament} strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" opacity="0.8"/>
              </svg>
              {/* Frise en haut */}
              <div style={{
                width: '100%', height: 1, marginBottom: 8,
                background: `linear-gradient(90deg, transparent, ${theme.frieze} 30%, ${theme.frieze} 70%, transparent)`,
              }} />
              <img
                loading="lazy"
                src={`/${folder}/${filename}`}
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <CommentCarousel images={COMMENT_IMAGES} folder="commentaires" />
        </motion.div>
        ...
        {/* ── Mini-section : témoignages Maths Ultime V1 ────────────────── */}
        <motion.div
        ...
        >
          <div className="flex items-center justify-center gap-3 mb-8">
            <div className="h-px w-10 md:w-20" style={{ background: 'linear-gradient(90deg, transparent, #c04a1e)' }} />
            <h3
              className="flex flex-wrap justify-center items-center gap-x-3 gap-y-3 uppercase text-center"
              style={{ fontFamily: 'var(--font-baloo)', fontWeight: 900, fontSize: 'clamp(18px, 3.2vw, 34px)', color: '#7a2a12', lineHeight: 1.05 }}
            >
              <span>ILS ONT</span>
              <span
                className="transform -rotate-2 inline-block"
                style={{ background: '#e0611a', color: '#fff8ee', padding: '3px 16px', border: '3px solid #7a2a12', boxShadow: '5px 5px 0 #7a2a12', fontFamily: 'var(--font-baloo)', fontWeight: 900 }}
              >
                AIMÉ
              </span>
              <span>MATHS ULTIME V1</span>
            </h3>
            <div className="h-px w-10 md:w-20" style={{ background: 'linear-gradient(90deg, #c04a1e, transparent)' }} />
          </div>

          <CommentCarousel images={V1_IMAGES} folder="temoignages-v1" reverse theme={ORANGE_STONE_THEME} />

          <p
            className="mt-8 text-center"
            style={{ fontFamily: 'var(--font-baloo)', fontWeight: 600, fontSize: 'clamp(12px, 1vw, 14px)', color: 'rgba(122,42,18,0.55)', letterSpacing: '.06em' }}
          >
            Retours réels d&apos;élèves sur la toute première version du guide
          </p>
        </motion.div>
      </div>
    </section>
